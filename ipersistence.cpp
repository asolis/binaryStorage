...
//Attributes
bool cv2::H5Utils::getAttribute(H5::H5Location &node, const H5::DataType &type, const std::string &name, void *value)
{
    hsize_t dims[] = {1};
...
void cv2::H5Utils::setAttribute(H5::H5Location &node, const H5::DataType &type, const std::string &name, const void *value)
{
    hsize_t dims[] = {1};
    H5::DataSpace attSpace(1, dims);
...
void cv2::H5Utils::setStrAttribute(H5::H5Location &node, const std::string &name, const char *value)
{
    std::vector<const char*> arr_c_str;
    arr_c_str.push_back(value);
    setAttribute(node, H5::StrType(H5::PredType::C_S1, H5T_VARIABLE), name, arr_c_str.data());
}
void cv2::H5Utils::setStrAttribute(H5::H5Location &node, const std::string &name, const std::string &value)
{
    setAttribute(node, H5::StrType(H5::PredType::C_S1, H5T_VARIABLE), name, value.c_str());
}
void cv2::H5Utils::setIntAttribute(H5::H5Location &node, const std::string &name, const int &num)
{
    setAttribute(node, H5::PredType::NATIVE_INT, name, &num);
}
bool cv2::H5Utils::getNodeType(H5::H5Location &node, int &value)
{
    return getAttribute(node, H5::PredType::NATIVE_INT, cv2::FileNode::TYPE, &value);
}
void cv2::H5Utils::setNodeType(H5::H5Location &node, int type)
{
...
void cv2::H5Utils::listSubnodes(H5::H5Location &node, std::vector<std::string> &subnodes)
{
    //herr_t idx =
...
{
    H5Utils::createGroup(*_bfs, fullName(name), type);
...
void cv2::H5FileWriter::write(const std::string &name, int data)
{
    ...
    cv2::H5Utils::writeDataset1D(*_bfs, H5::PredType::NATIVE_INT, name, &data, attr, 1);
}
void cv2::H5FileWriter::write(const std::string &name, float data)
{
    ...
    cv2::H5Utils::writeDataset1D(*_bfs, H5::PredType::NATIVE_FLOAT, name, &data,attr, 1);
}
void cv2::H5FileWriter::write(const std::string &name, double data)
{
    std::map<std::string,int> attr;
    attr.insert(std::make_pair(cv2::FileNode::TYPE, cv2::FileNode::REAL));
    cv2::H5Utils::writeDataset1D(*_bfs, H5::PredType::NATIVE_DOUBLE, name, &data,attr, 1);
}
void cv2::H5FileWriter::write(const std::string &name, const std::string &data)
{
    ...
    cv2::H5Utils::writeDataset1D(*_bfs, H5::StrType(H5::PredType::C_S1, H5T_VARIABLE), name, arr_c_str.data(), attr, 1);
}
...
void cv2::H5FileWriter::write(const std::string &name, const std::vector<int> &data)
{
    ...
    H5Utils::writeDataset1D(*_bfs, H5::PredType::NATIVE_INT, name, &data[0], attr, data.size());
}
void cv2::H5FileWriter::write(const std::string &name, const std::vector<float> &data)
{
    std::map<std::string,int> attr;
    attr.insert(std::make_pair(cv2::FileNode::TYPE, cv2::FileNode::SEQ));
    H5Utils::writeDataset1D(*_bfs, H5::PredType::NATIVE_FLOAT, name, &data[0], attr, data.size());
}
void cv2::H5FileWriter::write(const std::string &name, const std::vector<double> &data)
{
    std::map<std::string,int> attr;
    attr.insert(std::make_pair(cv2::FileNode::TYPE, cv2::FileNode::SEQ));
    H5Utils::writeDataset1D(*_bfs, H5::PredType::NATIVE_DOUBLE, name, &data[0], attr, data.size());
}
void cv2::H5FileWriter::write(const std::string &name, const std::vector<std::string> &vec)
{
    ...
    H5Utils::writeDataset1D(*_bfs, H5::StrType(H5::PredType::C_S1, H5T_VARIABLE), name, arr_c_str.data(), attr, vec.size());
}
void cv2::H5FileWriter::write(const std::string &name, const std::vector<cv::KeyPoint> &ks)
{
    ...
    H5::DataSet dataset(_bfs->createDataSet(name, mtype1, fspace));
...
{
    return cv2::H5Utils::readDataset(*_fn, name, cv2::FileNode::INT, &data);
}
bool cv2::H5FileReader::read(const std::string &name, float &data) const
{
    return cv2::H5Utils::readDataset(*_fn, name, cv2::FileNode::REAL, &data);
}
...
{
    H5::DataSet dataset = _fn->openDataSet(name);
    int _type;
...
{
    std::vector<int> _tmp;
    bool _read = cv2::H5Utils::readDataset1D<int>(*_fn, name, _tmp);
    if (!_read)
        return _read;
    _data.start = _tmp[0];
    _data.end = _tmp[1];
